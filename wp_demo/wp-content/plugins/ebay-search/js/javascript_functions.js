...
/* added by sahy */
function edit(id, table, pk){
    var req = ajax_file + '?ajaxAction=edit&id=' + id + '&table=' + table + '&pk=' + pk;    
    http.open('get', req);    
    http.onreadystatechange = function(){
        if(http.readyState == 4){			
            var return_string = http.responseText;
            var table_html = return_string;
            document.getElementById("div_edit_form_"+table).innerHTML = table_html;                                    
        }
    }
    http.send(null);    
}

function update_record(id, table, pk){    
    
    var req = ajax_file + '?ajaxAction=update_full&id=' + id + '&table=' + table + '&pk=' + pk;        
    //var parameters = getFormValues(document.getElementById("edit_form_"+table), '');    
    
    var parameters=$("#edit_form_"+table).serialize();  
